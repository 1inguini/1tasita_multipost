...
element("submit").addEventListener("click", () => {
  ...
  fetch("https://shikorism.net/api/v1/checkins", {
    ...
    body: {
      checked_in_at: `${element("date").value}T${
        element("time").value
      }:00+0900`,
      tags,
  ...
  fetch("https://nuita.net/nweets", {
    ...
    body: new URLSearchParams({
      ...
      "nweet[did_at]": `${element("date").value}T${
        element("time").value
      }:00+0900`,
      commit: "ヌイート",
    }).toString(),
...
function updateDate() {
  ...
  element("date").value = `${year}-${month}-${date}`;
  element("time").value = `${hour}:${minute}`;
}
updateDate();

element("isRealtime").addEventListener("change", function () {
  if (this.checked) updateDate();
  element("date").toggleAttribute("disabled", this.checked);
  element("time").toggleAttribute("disabled", this.checked);
});
...
function commitTag(tagString) {
  ...
  element("tags").appendChild(tag);
}
element("tagInput").addEventListener("input", function (event) {
  if (/\p{WSpace}/u.test(event.data)) {
    commitTag(this.value.trim());
    this.value = "";
  }
});
element("tagInput").addEventListener("change", function () {
  commitTag(this.value.trim());
  this.value = "";
});
